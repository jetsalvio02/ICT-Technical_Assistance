...
const Dashboard = () => {
  ...
  const { data: recentRequestsData, isLoading: isRequestsLoading } = useQuery({
    queryKey: ["recent-requests"],
    queryFn: async () => {
      const res = await fetch("/api/service-requests?limit=10");
      if (!res.ok) throw new Error("Failed to fetch requests");
      const data = await res.json();
      return data.data as RecentRequest[];
    },
  });
